...
System.out.println("---------------SCANNER PROVA----------------");


        Scanner scanner = new Scanner(System.in);
        System.out.println("\nBenvenuto nel sistema di trasporto pubblico!");
        ...
        switch (sceltaRuolo) {
            case 1:
                ...
                if (sceltaUtente == 1) {
                    ...
                    ticketDAO.save(new Ticket(LocalDate.now(), 1.50, vendor));
                    ...
                    System.out.println("Inserisci ID della card:");
                    Long cardId = scanner.nextLong();
                    Card card = cd.findById(cardId);

                    if (!cd.isCardValid(cardId)) {
                        System.out.println("Errore: la tessera non è valida o è scaduta.");
                        break; // esco dalla scelta utente
                    }

                    System.out.println("Inserisci data di inizio abbonamento (YYYY-MM-DD):");
                    scanner.nextLine(); // consuma newline
                    String dataStr = scanner.nextLine();
                    LocalDate startDate = LocalDate.parse(dataStr);  // attenzione al formato

                    System.out.println("Scegli tipo di abbonamento: 1. Settimanale, 2. Mensile");
                    int tipoScelto = scanner.nextInt();
                    PassType tipoPass = (tipoScelto == 1) ? PassType.WEEKLY : PassType.MONTHLY;

                    Pass nuovoPass = new Pass(card, startDate, tipoPass);
                    nuovoPass.setVendor(vendor);

                    pd.save(nuovoPass);
                    System.out.println("Pass creato con successo. Valido fino al: " + nuovoPass.getEndDate());
            ...
            case 2:
                System.out.println("\nHai scelto AMMINISTRATORE.");
                System.out.println("Inserisci ID del rivenditore:");
                Long vendorId = scanner.nextLong();

                System.out.println("Inserisci data INIZIALE (YYYY-MM-DD):");
                String fromStr = scanner.next();
                LocalDate fromDate = LocalDate.parse(fromStr);

                System.out.println("Inserisci data FINALE (YYYY-MM-DD):");
                String toStr = scanner.next();
                LocalDate toDate = LocalDate.parse(toStr);

                Vendor vendor = vd.findById(vendorId);
                List<Ticket> tickets = vd.findTicketsIssuedByVendorBetween(vendor, fromDate, toDate);

                System.out.println("Biglietti emessi:");
                for (Ticket t : tickets) {
                    System.out.println(" - ID: " + t.getTicketId() + ", Data: " + t.getDateOfPurchase() + ", Prezzo: " + t.getPrice());
                }
                break;
